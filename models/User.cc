...
const std::shared_ptr<int32_t> &User::getId() const noexcept { return id_; }
...
const std::vector<std::string> &User::insertColumns() noexcept {
    static const std::vector<std::string> inCols = {"id", "username",
                                                    "password"};
    return inCols;
}
...
bool User::validateMasqueradedJsonForCreation(
    const Json::Value &pJson,
...
bool User::validateMasqueradedJsonForUpdate(
    const Json::Value &pJson,
    const std::vector<std::string> &pMasqueradingVector, std::string &err) {
    if (pMasqueradingVector.size() != 3) {
        err = "Bad masquerading vector";
...
bool User::validJsonOfField(size_t index, const std::string &fieldName,
    ...
    case 1:
        ...
        // asString().length() creates a string object, is there any better way
